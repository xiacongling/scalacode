...
object s2000_2009 {
  @Solution(id = 2006, title = "差的绝对值为 K 的数对数目", difficulty = Difficulty.Easy, description =
  ...
  def countKDifference(nums: Array[Int], k: Int): Int = {
    val pre: mutable.HashMap[Int, Int] = mutable.HashMap()
    var cnt: Int = 0
    nums.foreach { n =>
      cnt += pre.getOrElse(n - k, 0)
      cnt += pre.getOrElse(n + k, 0)
      pre.updateWith(n)(oc => Some(oc.getOrElse(0) + 1))
    }
    cnt
  }
}
