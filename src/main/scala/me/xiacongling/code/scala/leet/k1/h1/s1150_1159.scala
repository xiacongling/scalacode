...
object s1150_1159 {
  private val daysOfMonth: Array[Int] = Array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31).scan(0)(_ + _)

  @Solution(id = 1154, title = "一年中的第几天", difficulty = Difficulty.Easy, description =
  ...
  def dayOfYear(date: String): Int = {
    ...
    val Array(y, m, d) = date.split("-").map(_.toInt)
    daysOfMonth(m - 1) + d + (if (isLeap(y) && m > 2) 1 else 0)
  }
}
