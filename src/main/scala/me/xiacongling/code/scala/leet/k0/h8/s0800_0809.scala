...
object s0800_0809 {
  @Solution(id = 807, title = "保持城市天际线", difficulty = Difficulty.Medium, description =
    """在二维数组grid中，grid[i][j] 代表位于某处的建筑物的高度。我们被允许增加
      |任何数量（不同建筑物的数量可能不同）的建筑物的高度。高度 0 也被认为是建筑物。
      |
  ...
  def maxIncreaseKeepingSkyline(grid: Array[Array[Int]]): Int = {
    ...
    val we: Array[Int] = Array.ofDim(m)
    val ns: Array[Int] = Array.ofDim(n)
    for (i <- 0 until m) {
      for (j <- 0 until n) {
        we(i) = we(i).max(grid(i)(j))
        ns(j) = ns(j).max(grid(i)(j))
      }
    }
    var result = 0
    for (i <- 0 until m) {
      for (j <- 0 until n) {
        result += we(i).min(ns(j)) - grid(i)(j)
      }
    }
    result
  }
