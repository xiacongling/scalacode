...
object s1330_1339 {
  @Solution(id = 1332, title = "删除回文子序列", difficulty = Difficulty.Easy, description =
  ...
  def removePalindromeSub(s: String): Int = {
    implicit class PalindromeExt(t: String) {
      def isPalindrome: Boolean = Range(0, t.length / 2).forall(i => t(i) == t(t.length - i - 1))
    }
    if (s.isPalindrome) 1 else 2
  }
}
